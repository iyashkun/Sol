...
async function monitorWallet(address, chatId) {
  ...
  solanaConnection.onAccountChange(pubKey, async (accountInfo) => {
    try {
      ...
      const txDetails = await solanaConnection.getTransaction(tx.signature, {
        commitment: 'confirmed',
        maxSupportedTransactionVersion: 0, // Support version 0 transactions
      });

      if (!txDetails) {
        console.warn(`No transaction details for ${tx.signature}`);
        return;
      }

      const { meta, transaction } = txDetails;
      const type = detectTransactionType(txDetails);
      ...
      bot.api.sendMessage(chatId, formatNotification(address, type, details, timestamp), {
        parse_mode: 'Markdown',
        reply_markup: keyboard,
      });
    } catch (error) {
      console.error(`Error processing transaction for ${address}:`, error.message);
      if (error instanceof solanaWeb3.SolanaJSONRPCError) {
        bot.api.sendMessage(chatId, `Error fetching transaction for ${address}: ${error.message}`);
      }
    }
  });
}
...
// Fetch token symbol (placeholder)
async function getTokenSymbol(mint) {
  return 'TOKEN_X'; // Replace with Birdeye/Helius API
}
...
// Bot commands (from previous response)
